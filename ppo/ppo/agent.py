...
class PPOAgent:
    def __init__(
        ...
        self.actor: Actor = Actor(
            input_neurons=self.state_size,
            fc_neuron_nums=fc_neuron_nums,
            output_neurons=self.action_size,
        ).to(self.device)
        self.critic: Critic = Critic(
            input_neurons=self.state_size, fc_neuron_nums=fc_neuron_nums
        ).to(self.device)

        self.actor_optimizer = optim.Adam(
            self.actor.parameters(), lr=alpha, amsgrad=True
        )
        self.critic_optimizer = optim.Adam(
            self.critic.parameters(), lr=alpha, amsgrad=True
        )
    ...
    def remember(self, state, action, prob, value, reward, done):
        transition = Transition(
            state=state, action=action, prob=prob, value=value, reward=reward, done=done
        )
    ...
    def save_model(self, env_name: str, checkpoint=""):
        ...
        #     f"trained_models/{env_name}_{checkpoint}_actor.ckpt",
        # )
        # torch.save(
        #     self.critic.state_dict(),
        #     f"trained_models/{env_name}_{checkpoint}_critic.ckpt",
        # )
        pass
