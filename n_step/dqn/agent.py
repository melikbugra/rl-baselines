...
class DQNAgent:
    def __init__(self, env: gym.Env, device: torch.device, episodes: int, learning_rate: float, 
                 gamma: float, batch_size: int, epsilon_start: float, epsilon_end: float, 
                 exploration_percentage: float, fc_num: int, fc_neuron_nums: list[int], n_step: int = 5):
        self.env = env
        ...
        self.memory = ReplayMemory(self.state_size, 1, 10000, self.batch_size, n_step=n_step, gamma=gamma)
    ...
    def plot_scores(self, show_result=False):
        plt.figure("N-Step DQN")
        scores = torch.tensor(self.episode_scores, dtype=torch.float)
        if show_result:
            plt.title('N-Step DQN Result')
        else:
            plt.clf()
            plt.title('Training N-Step DQN...')
        plt.xlabel('Episode')
        plt.ylabel('Score')
    ...
    def optimize_model(self):
        ...
        if self.use_n_step:
            ...
            with torch.no_grad():
                next_state_values = self.target_net(next_state_batch).max(1)[0]
            # Compute the expected Q values
            expected_state_action_values = (next_state_values * gamma * mask.squeeze(1)) + reward_batch.squeeze(1)

            n_loss = criterion(state_action_values, expected_state_action_values.unsqueeze(1))
