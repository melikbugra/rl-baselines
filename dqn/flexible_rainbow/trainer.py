...
class Trainer:
    def __init__(self, env_name: str, render: bool, episodes: int, batch_size: int, 
        ...
        # ]
        # self.env = gym.make(self.env_name, maze=maze, max_steps=500)
        self.env = gym.make(self.env_name, max_steps=200)
    ...
    def train(self, trial_num = -1):
        ...
        for episode in range(self.episodes):
            ...
            for t in count():
                ...
                if self.agent.noisy_enabled:
                    action = self.agent.select_greedy_action(state)
                else:
                    action = self.agent.select_e_greedy_action(state)
...
if __name__ == "__main__":
    trainer = Trainer(
        env_name="SimpleMaze-v1", render=False, episodes=3000, batch_size=64, gamma=0.99, epsilon_start=1, epsilon_end=0.001, exploration_percentage=80, learning_rate=3e-4, 
        fc_num=1, fc_neuron_nums=[256,256], tau=0.005, device="cpu")
    trainer.train()
